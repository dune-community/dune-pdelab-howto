...
\item However, the number of cores is increasing. Quad-cores are the rule,
...
\item High-performance computer have up to 1'572'864 cores (Sequoia - Bluegene/Q, Livermore, USA)
or 3'120'000 cores (Tianhe-2, Guangzhou, China, 384'000 Intel Xeon, 2'736'000 Intel Phi) .
\end{itemize}
...
same amount of time (most multi-core CPUs)
...
\item Free Implementations also for LINUX Clusters: {\bfseries MPICH}
 and {\bfseries OpenMPI} \footnote{\tiny \href{http://www-unix.mcs.anl.gov/mpi/mpich}{http://www-unix.mcs.anl.gov/mpi/mpich} and \href{http://www.open-mpi.org/}{http://www.open-mpi.org/}}
...
\begin{frame} \frametitle{Recapitulation: The Grid: A Container of Entities}
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\linewidth-5mm}
      \begin{onlyenv}<presentation>
        \includegraphics<1-2>[width=\linewidth]{EPS/entities}
        \includegraphics<3->[width=\linewidth]{EPS/entities_cd}
      \end{onlyenv}
      \begin{onlyenv}<article>
        \vspace*{2ex}
        ~\hfill\includegraphics[width=0.4\linewidth]{EPS/entities}\hfill
        \includegraphics[width=0.4\linewidth]{EPS/entities_cd}\hfill~
      \end{onlyenv}
    \end{column}\hfill
...
\begin{frame}
  ...
  \item<2-> \lstinline!UGGrid!
      ...
      \item multi-element
      \item ghost cells
      \item non-conforming/conforming (red-green) refinement
      \end{itemize}
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{0.47\linewidth}
      \begin{center}
        \begin{onlyenv}<article>
          \includegraphics[width=0.7\linewidth]{EPS/iterators}
        \end{onlyenv}
      \end{center}
      \begin{onlyenv}<presentation>
        \includegraphics[width=\linewidth]{EPS/iterators}
      \end{onlyenv}
    \end{column}
...
\begin{frame}
  ...
  \lstinputlisting[breaklines=true]{src_examples/iterators.cc}
\end{frame}
...
%  \begin{block}{}%
    \only<presentation>{\lstset{basicstyle=\scriptsize\ttfamily}}
      \lstinputlisting[breaklines=true]{src_examples/entity.cc}%
%  \end{block}
    ...
    \begin{column}{0.4\textwidth}
      \begin{center}
        \visible<presentation|1->{
          \includegraphics[width=\linewidth]{EPS/refelem-mapping}}\\
        \begin{onlyenv}<article>
          \includegraphics[width=0.6\linewidth]{EPS/refelem-mapping}\\
        \end{onlyenv}
...
\begin{frame} \only<presentation>{\frametitle{Intersections}}
  ...
  \begin{columns}
    \begin{column}{0.4\linewidth}
      \begin{center}
        \includegraphics<presentation>[width=\linewidth]{EPS/intersection}
        \includegraphics<article>[width=0.45\linewidth]{EPS/intersection}
      \end{center}
    \end{column}
    \begin{column}{0.6\linewidth}
      \begin{itemize}
...
%      \item The \lstinline!GridView! provides access to the IntersectionIterator:
        \begin{itemize}
...
\begin{frame} \frametitle{Intersection Interface}
  Dereferencing \lstinline!IntersectionIterator! yields \lstinline!Intersection!:\bigskip
...
\only<presentation>{\lstset{basicstyle=\scriptsize\ttfamily}}
  \lstinputlisting[breaklines=true]{src_examples/intersection.cc}
\end{frame}

\begin{frame} \frametitle{Example II}
\only<presentation>{\lstset{basicstyle=\scriptsize\ttfamily}}
  \lstinputlisting[breaklines=true]{src_examples/intersection2.cc}
\end{frame}
...
\subsubsection{DUNE Lowlevel Communication API}
\begin{frame}[fragile]
...
The class  \lstinline!Dune::Grid::CollectiveCommunication! provides comfortable access to a lot of MPI methods, e.g.
...
\lstinputlisting[lastline=12]{src_examples/communicate.cc}
\end{frame}
\subsubsection{MPIHelper}
\begin{frame}[fragile]
...
\lstinputlisting[lastline=12]{../../src/course-examples/example01_main.hh}
